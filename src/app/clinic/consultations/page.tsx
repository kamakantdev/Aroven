...
export default function ClinicConsultationsPage() {
    ...
    const loadConsultations = async () => {
        setLoading(true);
        try {
            const res = await clinicApi.getConsultations(page, statusFilter || undefined) as any;
            if (res.success) {
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto">
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Clinic Consultations</h1>
                    <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">View consultations by your clinic doctors</p>
                </div>
                                    ...
                                    <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            {c.status === 'in_progress' && (c.type === 'video' || c.appointment?.type === 'video') && (
                                                <Link href={`/consultation/${c.id}`} className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-white bg-pink-600 hover:bg-pink-700 rounded-lg transition-colors">
